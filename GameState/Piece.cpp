...
struct PieceTraits
{
//	UINT	m_BlackResourceId;
//	UINT	m_WhiteResourceId;
	char	m_Symbol[2];
};

PieceTraits const s_PieceTraits[ NUMBER_OF_PIECE_TYPES ] =
{
...
//							s_PieceTraits[ t ].m_WhiteResourceId :
//							s_PieceTraits[ t ].m_BlackResourceId;
//	int	result = image_->LoadBitmap( image_resid );
//	ASSERT( result != 0 );

	symbol_	= s_PieceTraits[ (size_t)t ].m_Symbol;
}
...
char const * Piece::symbol( PieceTypeId id )
{
	...
	{
		return s_PieceTraits[(size_t)id].m_Symbol;
	}
	else
	{
		return "?";
...
void Pawn::generatePossibleMoves( GameState const & state, Position const & from, MoveList & moves ) const
{
	...
	if ( board.isValidPosition( to ) )
	{
		...
		else	// Note: If a piece can be captured normally, then it is impossible for a pawn to have moved two
